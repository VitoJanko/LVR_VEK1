...
def SAT(cnf,spr,cs):
    #SAT(cnf, spr, cs) je funkcija, ki reši SAT za podan slovar spremenljivk z že določenimi vrednostmi in seznam spremenljivk, ki še nimajo določenih vrednosti.
    #spr=spremenljivke, cs=seznam spremenljivk, ki nimajo se vrednosti
    found = True
                                #Najprej rešimo izraze v cnf z eno spremenljivko z določeno vrednostjo ali z nič spremenljivkami.
    while found: 
                ...
                if formula[0][0]=='-':
                    ...
                    if formula[0] in cs:
                        cs.remove(formula[0])
                                ...
                                #Določimo vrednost spremenljivk, ki še nimajo določenih vrednosti. 
    if len(cnf)==0:
        return spr
    else:
        neReseno = cs[0]
        ...
        for f in formula:
            ...
            if f[1:] in spr.keys():         #spremenljivka brez negacije je že v slovarju spr
                if spr[f[1:]]:
                    ...
                    isTrue = True
                    found= True
            if not found:                   #podsez je seznam vseh spremenljivk, ki še nimajo določene vrednosti
                podsez.append(f)
        if not isTrue:                  
            sez.append(podsez)
    return sez
...
def resi(izraz):
    #RESI(izraz) je funkcija, ki reši SAT s pomočjo funkcije SAT(cnf,spr,cs), in vrne seznam vrednosti spremenljivk, za katere je izraz resničen.
    izraz = cnf(izraz)
